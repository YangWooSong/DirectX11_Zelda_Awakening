...
//그림자 그리는 렌더타겟 사이즈 (최대한 그게 만들어야 상세함)
_uint		g_iSizeX = 8192;
_uint		g_iSizeY = 4608;
...
HRESULT CRenderer::Initialize()
{
	...
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Diffuse"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(1.f, 1.f, 1.f, 0.f))))
		return E_FAIL; //알파를 0으로 초기화 -> 모델이 그려지지 않는 부분은 Diferred Shader에서 알파를 이용해 잘라낸다.
	...
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Bloom"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(0.f, 0.f, 0.f, 1.f))))
		return E_FAIL; //Bloom 오브젝트 띄우는 용
	...
	if (FAILED(m_pGameInstance->Add_MRT(TEXT("MRT_GameObjects"), TEXT("Target_Depth"))))
		return E_FAIL;	//게임 오브젝트를 그릴 때 깊이를 기록
...
HRESULT CRenderer::Render_Deferred()
{
	...
	//XMStoreFloat4x4(&ViewMatrix, XMMatrixLookAtLH(XMVectorSet(5.f, 30.f, -15.f, 0.f), m_pGameInstance->Get_LightLook_Vector(), XMVectorSet(0.f, 1.f, 0.f, 0.f)));
...
HRESULT CRenderer::Render_Bloom_Final()
{
	//if (FAILED(m_pGameInstance->Begin_MRT(TEXT("MRT_Final"))))
	//	return E_FAIL;
	...
	if (FAILED(m_pGameInstance->Bind_RT_ShaderResource(m_pShader, TEXT("Target_Final"), "g_BackTexture")))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Bind_RT_ShaderResource(m_pShader, TEXT("Target_Blur_Y"), "g_BlurYTexture")))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Bind_RT_ShaderResource(m_pShader, TEXT("Target_Bloom"), "g_BloomTexture")))
